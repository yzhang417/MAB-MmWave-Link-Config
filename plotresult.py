...
for exid in [2,3,5]: # Scenario 1,2,3 in paper
    ...
    # Only exid 1, 2 and 3 has genius, worst and optimal policy
    if exid == 2 or exid == 3:
        ave_reward_fixed_policy = ave_reward[:-3];
        ave_unachievable_optimal_ave_evolution_reward = np.mean(np.amax(evolution_reward[:,:,:-3],axis=2),axis=0)/1e9
    else:
        ave_reward_fixed_policy = ave_reward[:-2];
        ave_unachievable_optimal_ave_evolution_reward = np.mean(np.amax(evolution_reward[:,:,:-2],axis=2),axis=0)/1e9
    ...
    if exid == 4 or exid == 5:
        ...
        policy_setting_list[0].legend = 'Random selection of beam sweeping period\nand beamwidth with all beams';
        policy_setting_list[0].color = 'C0';
        policy_setting_list[1].legend = 'Bandit-based MCTS policy\nselecting all beams (R = 1)';
        policy_setting_list[1].color = 'C1';
        policy_setting_list[2].legend = 'Bandit-based MCTS policy\nselecting 1/2 of all beams (R = 1/2)';
        policy_setting_list[2].color = 'C2';
        policy_setting_list[3].legend = 'Bandit-based MCTS policy\nselecting 1/4 of all beams (R = 1/4)';
        ...
        policy_to_be_shown = [genius_policy_id, worst_policy_id, len(policy_setting_list)-3, len(policy_setting_list)-2, len(policy_setting_list)-1];
        #policy_to_be_shown = [genius_policy_id, worst_policy_id, len(policy_setting_list)-3, len(policy_setting_list)-2];
        genius_tslot = t_slot_vec[int(policy_setting_list[genius_policy_id].default_tslot_id)]
        worst_tslot = t_slot_vec[int(policy_setting_list[worst_policy_id].default_tslot_id)]
        policy_setting_list[genius_policy_id].legend = 'Genius policy (beam sweeping period: ' + str(genius_tslot) + 'ms)'
        policy_setting_list[genius_policy_id].color = 'c'
    ...
    figID = exid*100
    plt.figure(num=figID,figsize=(9,7),dpi=1200)
    # Plot optimal data rate via exhasutive search for scenario 2 and 3
    if exid == 2:
        plt.plot(range(numSlotToShow),ave_unachievable_optimal_ave_evolution_reward[0:numSlotToShow],label='Optimal data rate (exhaustive search per time slot)',c='C6')
    if exid == 3:
        plt.plot(range(numSlotToShow),ave_unachievable_optimal_ave_evolution_reward[0:numSlotToShow],label='Optimal data rate \n(exhaustive search per time slot)',c='C6')
    # Plot schemes
    for i in policy_to_be_shown:
        policy_setting = policy_setting_list[i];
        plt.plot(range(numSlotToShow),ave_evolution_reward[0:numSlotToShow,i],label=policy_setting_list[i].legend,c=policy_setting_list[i].color)
    plt.legend()
    plt.xlabel('Time slot index')
    plt.ylabel('Data rate (Gbps)');
    if exid == 2:
        plt.legend(loc=(0.1, 0.02))
    elif exid == 3:
        plt.legend(loc=(0.1, 0.02))
    else:
        plt.legend(loc=(0.1, 0.02))
    plt.savefig(output_folder+'example'+str(exid)+'.pdf',format='pdf', facecolor='w', transparent=True, dpi=1200)
    ...
    figID = exid*101
    plt.figure(num=figID,figsize=(9,6),dpi=1200);   
    plt.axis('equal')
