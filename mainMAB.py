...
def main():
    ...
    # Initialization
    #----------------------------------------------
    slots_monitored = args.slots;         # Total time slots to be monitored in each reaelization
    LOOP = args.LOOP;                     # Number of realization
    exid = args.exid;                     # Example ID
    ...
    evolution_bw = np.zeros((LOOP,slots_monitored,N_polices));      # Evolution of bw selected of the whole training process
    evolution_tslot = np.zeros((LOOP,slots_monitored,N_polices));   # Evolution of beam training period selected of the whole training process
       
    #-----------------------------------------------------------
    # Random seed
    #-----------------------------------------------------------
    random.seed(13579)     # random seeds for reproduction
    np.random.seed(246810) # random seeds for reproduction
            ...
            for policy_id in range(N_polices):
                ...
                if ct > 0:
                    ...
                    if exid == 4 and policy_setting_list[policy_id].enable_bandit:
                        evolution_reward[loop,ct,policy_id] = state_list[policy_id].new_reward;
                    else:
                        evolution_reward[loop,ct,policy_id] = reward;
                    evolution_bw[loop,ct,policy_id] = action.bw_id;   
                    evolution_tslot[loop,ct,policy_id] = action.tslot_id;
